...
// 参数:
//   value:
//     要查找的 System.String 对象。
//
// 返回结果:
//     如果 value 参数出现在此字符串中，或者 value 为空字符串 ("")，则为 true；否则为 false。
function RegStrContains(str1,str2)
{
if(str2.toString()=="")
    return true;
...
//验证url合法"http://www.****.com"
function  IsUrl(str) 
{ 
    var  regUrl  =  /(http\:\/\/)(\/[\w-  \.\/\?%&=]*)?/gi; 
...
var checkData = new function() {   
    var idExt="_BYT_Span";//生成span层的id后缀   
    /**  
    * 得到中英文字符长(中文为2个字符)  
    *  
    * @param {}  
    ...
    * @param {}  
    *            str  
    * @return {Boolean} true：手机号，false:<b>不是</b>手机号;  
    */  
    this.isMobile = function(str) {   
        var re = /^0{0,1}(13[0-9]|15[0-9]|18[8|9])[0-9]{8}$/;   
        return re.test(str);   
    }   
  
    this.isMobileSpan = function(id) {   
        var val = document.getElementById(id);   
        if (!this.isMobile(val.value)) {   
            val.select();   
            var str = "手机号不符合格式;";   
            this.appendError(id, str);   
            return false;   
